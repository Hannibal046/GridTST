...
class GridTSTSelfAttention(nn.Module):
...
class GridTSTEncoderLayer(nn.Module):
    def __init__(self,config):
        super().__init__()
        self.num_channels = config.num_channels

        ## Attention
        self.self_attention = GridTSTSelfAttention(config)
...
class GridTSTFlattenHead(nn.Module):
    def __init__(self,config):
        super().__init__()
...
class GridTSTPreTrainedModel(PreTrainedModel):
    config_class = GridTSTConfig
...
class GridTSTEncoder(GridTSTPreTrainedModel):
    def __init__(self,config):
        super().__init__(config)
        self.num_channels = config.num_channels

        self.layers = nn.ModuleList(
            GridTSTEncoderLayer(config) for _ in range(config.num_layers)
        )
        if config.attention_strategy == 'alternate':
            self.channel_layer_ids = list(range(0,config.num_layers,2))
            self.time_layer_ids = list(range(1,config.num_layers,2))
        elif config.attention_strategy == 'channel_first':
            self.channel_layer_ids = list(range(0,int(config.num_layers/2)))
            self.time_layer_ids = list(range(int(config.num_layers/2),config.num_layers))
        elif config.attention_strategy == 'time_first':
            self.channel_layer_ids = list(range(int(config.num_layers/2),config.num_layers))
            self.time_layer_ids = list(range(0,int(config.num_layers/2)))
    ...
    def forward(self,hidden_states):
        ...
        for idx,layer in enumerate(self.layers):
            if idx in self.time_layer_ids:
                hidden_states = layer(hidden_states)
            elif idx in self.channel_layer_ids:
                # _,seq_len,d_model = hidden_states.shape
                hidden_states = hidden_states.reshape(-1,num_channels,seq_len,d_model)
                hidden_states = hidden_states.permute(0,2,1,3)
                hidden_states = hidden_states.reshape(-1,num_channels,d_model)

                hidden_states = layer(hidden_states)

                hidden_states = hidden_states.reshape(-1,seq_len,num_channels,d_model)
                hidden_states = hidden_states.permute(0,2,1,3)
                hidden_states = hidden_states.reshape(-1,seq_len,d_model)
        return hidden_states
        
class GridTSTModel(GridTSTPreTrainedModel):
    def __init__(self,config):
        super().__init__(config)
        ...
        self.dropout = nn.Dropout(config.dropout)
        self.encoder = GridTSTEncoder(config)
        self.post_init()
...
class GridTSTForTimeSeriesPrediction(GridTSTPreTrainedModel):
    def __init__(self,config):
        super().__init__(config)
        ...
        self.model = GridTSTModel(config)
        self.head = GridTSTFlattenHead(config)
        self.post_init()
    ...
    def forward(
        ...
        # hidden_states = self.model(patched_input_values.reshape(batch_size*num_channels,num_patch,patch_len)) # [bs*num_channels,num_patches,patch_len]
